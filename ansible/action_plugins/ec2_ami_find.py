    ...
    def run(self, conn, tmp, module_name, module_args, inject, complex_args=None, **kwargs):
        ...
        if region is None:
            return ReturnData(conn=conn, comm_ok=True,
                result=dict(
                    failed=True, changed=False,
                    msg='region is a required argument'
                )
            )
        role_credentials = args.get('role_credentials', None)
        if role_credentials is None:
            return ReturnData(conn=conn, comm_ok=True,
                result=dict(
                    failed=True, changed=False,
                    msg='Internal error: role_credentials not specified'
                )
            )

        ami_id = args.get('ami_id', None)
        ami_tags = args.get('ami_tags', None)
        architecture = args.get('architecture', None)
        hypervisor = args.get('hypervisor', None)
        is_public = args.get('is_public', None)
        name = args.get('name', None)
        owner = args.get('owner', None)
        platform = args.get('platform', None)
        sort = args.get('sort', None)
        sort_tag = args.get('sort_tag', None)
        sort_order = args.get('sort_order', 'ascending')
        sort_start = args.get('sort_start', None)
        sort_end = args.get('sort_end', None)
        state = args.get('state', 'available')
        virtualization_type = args.get('virtualization_type', None)
        no_result_action = args.get('no_result_action', 'success')
        ...
        # args.update(
        #     {
        #         'aws_access_key': role_credentials['AWS_ACCESS_KEY_ID'],
        #         'aws_secret_key': role_credentials['AWS_SECRET_ACCESS_KEY']
        #     }
        # )
        # ec2conn = boto.ec2.connect_to_region(region, **args)
        ec2conn = boto.ec2.connection.EC2Connection(
            ...
            security_token=role_credentials['AWS_SECURITY_TOKEN'])
        # 
        # getting:
        ...
        # 
        images_result = ec2conn.get_all_images(owners=owner, filters=filter)

        if no_result_action == 'fail' and len(images_result) == 0:
            return ReturnData(conn=conn, comm_ok=True,
                result=dict(
                    failed=True, changed=False,
                    msg="No AMIs matched the attributes: %s" % json.dumps(filter)
                )
            )
